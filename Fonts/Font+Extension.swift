//
//  Font+Extension.swift
//  HeilpraktikerLernen (iOS)
//
//  Created by Guido Kühn on 13.07.21.
//

import Foundation
import SwiftUI

extension Font {
    static let myText = Font.custom("Raleway-Regular", size: 16)
    static let myHeader = Font.custom("Raleway-SemiBold", size: 16)
    static let myTitle = Font.custom("Raleway-SemiBold", size: 24)
    static let myTextItalic = Font.custom("Raleway-Italic", size: 16)
    static let myLabel = Font.custom("Raleway-Italic", size: 8)
}
